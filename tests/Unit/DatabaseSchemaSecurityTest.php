...
it('returns expired short codes to the reusable pool', function () {
    $schema = file_get_contents(dirname(__DIR__, 2) . '/scripts/db.sql');
...
it('does not maintain a separate lifetime issuance metric', function () {
    ...
    expect($schema)->not()->toContain('CREATE TABLE `clip_metrics`')
        ->and($migration)->not()->toContain('CREATE TABLE `clip_metrics`');
});
